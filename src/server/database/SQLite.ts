...
export class SQLite implements IDatabase {
  ...
  saveGameResults(gameId: GameId, players: number, generations: number, gameOptions: GameOptions, scores: Array<Score>, game: Game): void {
    ...
    this.db.run(
      ...
      [gameId, gameOptions.clonedGamedId, players, generations, JSON.stringify(gameOptions), JSON.stringify(scores), gameJSON], (err) => {
        if (err) {
  ...
  async cleanGame(gameId: GameId): Promise<void> {
    try {
      ...
      await this.asyncRun('INSERT into purges (game_id, last_save_id) values (?, ?)', [gameId, saveId]);
