...
export class SQLite implements IDatabase {
  ...
  saveGameResults(gameId: GameId, players: number, generations: number, gameOptions: GameOptions, scores: Array<Score>, game: IGame): void {
    ...
    try {
      this.db.prepare(
        'INSERT INTO game_results (game_id, seed_game_id, players, generations, game_options, scores, game) VALUES(?, ?, ?, ?, ?, ?, ?)',
      ).run([gameId, gameOptions.clonedGamedId, players, generations, JSON.stringify(gameOptions), JSON.stringify(scores), gameJSON]);
    } catch (err) {
